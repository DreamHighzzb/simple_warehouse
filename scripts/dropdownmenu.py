...
class FilteredMenu(ttk.Frame):
    ...
    def __init__(self, parent, values=None, width=25, height=8, 
                 label_text=None, theme='light', call_back = None, call_back_param = "", **kwargs):
        """
        初始化美化样式的筛选下拉框
    ...
    def _create_widgets(self, label_text):
        ...
        self.entry.pack(side=tk.LEFT, padx=(0, 0))
        self.entry_var.set("▼")
        ...
        self.button_frame.pack(side=tk.LEFT, padx=(0, 0))
        ...
        #     cursor='hand2'
        # )
        # self.dropdown_btn.pack(side=tk.LEFT, padx=(0, 0), pady=0)
    ...
    def _select_value(self, index):
        ...
        if 0 <= index < len(self.filtered_values):
            ...
            if None != self.call_back:
                self.call_back(value, self.call_back_param)
            self._hide_dropdown()
...
# 使用示例
def createFilteredMenu(main_frame, products, width, height, text, cb, param):
    """美化样式示例"""
    combobox = FilteredMenu(
        main_frame,
        values=products,
        width=width,
        height = height,
